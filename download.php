...
require_once 'vendor/autoload.php'; // Ensure the correct path to Composer's autoload file
...
// Initialize DOMPDF
$options = new Options();
$options->set('isHtml5ParserEnabled', true); // Enable HTML5 parsing for better CSS support
$options->set('isPhpEnabled', true); // Enable PHP if needed (use carefully)
$dompdf = new Dompdf($options);

$bootstrapCSS = file_get_contents('https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css');

$html = $bootstrapCSS;
$html = file_get_contents('./template/modern_template.php');
$dompdf->loadHtml($html);

// (Optional) Set paper size (default is A4)
$dompdf->setPaper('A4', 'portrait');

// Render PDF (first pass: full page)
$dompdf->render();

// Output the generated PDF to the browser for download
$dompdf->stream('example.pdf', array('Attachment' => 1)); // Attachment => 1 forces download
